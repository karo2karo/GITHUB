...
class SettingsTab:
    def __init__(self, parent, currency_converter, tip_operations):
        ...
        self.setup_tab()
        self.load_settings()  # Load settings after UI is setup
    ...
    def setup_tab(self):
        ...
        export_frame.grid(column=0, row=4, columnspan=2, sticky=(tk.W, tk.E), pady=20)
        
        # Include zero values checkbox
        self.include_zero_days_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            export_frame, 
            text="Include zero values for missing dates in CSV exports", 
            variable=self.include_zero_days_var
        ).grid(column=0, row=0, sticky=tk.W, pady=5)
        
        # Explanation text
        ttk.Label(
            export_frame, 
            text="When enabled, exports will include entries with 0 amount for dates with no tips"
        ).grid(column=0, row=1, sticky=tk.W, pady=5)
    
    def load_settings(self):
        """Load settings from database."""
        settings = self.currency_converter.db_manager.settings_collection.find_one({"_id": "app_settings"})
        
        if settings:
            # Load export settings if available
            if 'include_zero_days_export' in settings:
                self.include_zero_days_var.set(settings['include_zero_days_export'])
    ...
    def save_settings(self):
        ...
        # Get the export setting
        include_zero_days = self.include_zero_days_var.get()
        
        # Save all settings to database
        self.currency_converter.db_manager.settings_collection.update_one(
            {"_id": "app_settings"},
            {"$set": {
                "base_currency": new_base,
                "include_zero_days_export": include_zero_days
            }},
            upsert=True
        )
        
        # Handle base currency change if needed
